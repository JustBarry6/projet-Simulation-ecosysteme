import java.util.ArrayList;

public class Sauterelle extends Insecte {

    public Sauterelle() {
        this.proies = new ArrayList<>();
        this.predateurs = new ArrayList<>();
    }

    @Override
    public void seDeplacer() {
        // implémentation de la méthode seDeplacer pour les sauterelles
    }

    @Override
    public void manger() {
        // implémentation de la méthode manger pour les sauterelles
    }

    @Override
    public void boire() {
        // implémentation de la méthode boire pour les sauterelles
    }

    @Override
    public void mourir() {
        // implémentation de la méthode mourir pour les sauterelles
    }

    @Override
    public void seReproduire() {
        // implémentation de la méthode seReproduire pour les sauterelles
    }
}
